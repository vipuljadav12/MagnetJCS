...
<div class="tab-pane fade show active" id="preview03" role="tabpanel" aria-labelledby="preview03-tab">
    <form action="{{url('/admin/LateSubmission/Process/Selection/'.$application_id.'/store')}}" method="post" id="process_selection">
             {{csrf_field()}}
       ...
       <table class="table m-0" id="tbl_population_changes">
                <thead>
                    <tr>
                        ...
                        <th class="text-center d-none" style="position: sticky; top: 0; background-color: #fff !important; z-index: 9999 !important">White</th>
                        <th class="text-center" style="position: sticky; top: 0; background-color: #fff !important; z-index: 9999 !important">Non-Black</th>
                    ...
                    @if($display_outcome == 1)
                        @foreach($waitlist_process_logs as $key=>$value)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">{{$value->late_application_count}}</td>
                    ...
                    @else
                        @isset($disp_arr)
                            ...
                            @foreach($disp_arr as $key=>$value)
                                <tr class="bg-info text-white"><td colspan="12">{{$key}}</td></tr>
                                    ...
                                    <tr class="{{$class}}" id="row{{$wvalue['application_program_id']}}">
                                        ...
                                        <td class="text-center">
                                            <span class="{{$spanclass}}">
                                                <select class="form-control" style="width:100px;" onchange="enableDisableWithdrawn({{$count}}, $(this).val(), {{$wvalue['application_program_id']}})" name="withdrawn_student{{$wvalue['application_program_id']}}">
                                        ...
                                        <td>
                                            <span class="{{$spanclass}}">
                                                <input type="text" class="form-control text-center" style="width: 100px;" id="black{{$count}}" value="{{$wvalue['black_withdrawn']}}" @if($spanclass=="hide-table" || $wvalue['withdrawn_student'] == "No") disabled @endif onkeypress="return onlyNumberKey(event)" name="black{{$wvalue['application_program_id']}}">
                                            </span>
                                        </td>
                                        <td class="d-none">
                                            <span class="{{$spanclass}}">
                                                <input type="text" class="form-control text-center" style="width: 100px;" id="white{{$count}}" value="{{$wvalue['white_withdrawn']}}"  @if($spanclass=="hide-table" || $wvalue['withdrawn_student'] == "No") disabled @endif onkeypress="return onlyNumberKey(event)" name="white{{$wvalue['application_program_id']}}">
                                            </span>
                                        </td>
                                        <td>
                                            <span class="{{$spanclass}}">
                                                <input type="text" class="form-control text-center" style="width: 100px;" id="other{{$count}}" value="{{$wvalue['other_withdrawn']}}"  @if($spanclass=="hide-table" || $wvalue['withdrawn_student'] == "No") disabled @endif onkeypress="return onlyNumberKey(event)" name="other{{$wvalue['application_program_id']}}">
                                            </span>
                                        </td>
                                        ...
                                        <td class="text-center"><span class="{{$spanclass}}">{{ $wvalue['late_application_count'] }}</span>
                                            <input type="hidden" value="{{ $wvalue['late_application_count'] }}" name="late_application_count{{$wvalue['application_program_id']}}">
                                        </td>
                                     ...
                                     @if(!empty($wvalue['zoned_schools']))
                                       
                                        <tr class="hzs_container_{{$wvalue['application_program_id']}}  @if($wvalue['withdrawn_student'] == "No") d-none @endif" id="hzs_container_{{$wvalue['application_program_id']}}">
                                            <td colspan="11">
                                                <div class="col-lg-12 row"> 
                                        @foreach($wvalue['zoned_schools'] as $hzs_key=>$hzs_chunk)
                                               
                                              <div class="col-lg-3 pt-5 inline-block">
                                                <div class="card">
                                                    <div class="card-body">
                                                        <p class="text-center"><strong>{{$hzs_key}}</strong></p>
                                                        <p style="display: inline-block; width: 50%" class="pr-5">
                                                        Black <input type="text" name="zoned_schools[{{$wvalue['application_program_id']}}][{{$hzs_key}}][black]" class="form-control text-center hzs_val" onkeypress="return onlyNumberKey(event)" style="width: 50px; display: inline-block; " value="{{($wvalue['zoned_schools'][$hzs_key]['black'] ?? 0)}}"></p>
                                                        <p  style="display: inline-block">  Non-Black <input type="text" name="zoned_schools[{{$wvalue['application_program_id']}}][{{$hzs_key}}][non_black]" class="form-control text-center hzs_val" onkeypress="return onlyNumberKey(event)" style="width: 50px;display: inline-block; " value="{{($wvalue['zoned_schools'][$hzs_key]['non_black'] ?? 0)}}"> </p>
                                                    </div>
                                                    </div>
