    ...
    @include("layouts.admin.common.alerts")
        ...
        <ul class="nav nav-tabs" id="myTab1" role="tablist">
            @foreach($statusArr as $key=>$value)
                ...
                @else
                    <li class="nav-item">
                        <a class="nav-link maintab" href="{{url('/admin/LateSubmission/EditCommunication/application/'.$application_id.'/'.$key)}}">{{$value}}</a>
                    </li>
                @endif
            @endforeach
                ...
                <ul class="nav nav-tabs custom-nav-tabs" id="myTab2">
                            ...
                            <div class="tab-content tab-validate  bordered" id="myTab2Content">
                                ...
                                <div class="tab-pane fade show active" id="active-screen">
                                     <form class="" action="{{url('admin/LateSubmission/EditCommunication/store/letter')}}" method="POST">
                                    {{csrf_field()}}
                                    ...
                                    <div class="form-group">
                                        <label style="width: 100%">Letter Body : @if(isset($data->letter_body) && $data->letter_body != '')<a href="{{url('/admin/LateSubmission/EditCommunication/preview/letter/'.$status.'/'.$application_id)}}" target="_blank" class="btn btn-success" style="float: right !important;">Preview</a>@endif</label>
                                ...
                                <div class="tab-pane fade @if($status == "ContractLetterText") show active @endif" id="active-email">
                                    <form class="" action="{{url('admin/LateSubmission/EditCommunication/store/email')}}" method="POST">
                                    {{csrf_field()}}
                                    <input type="hidden" name="application_id" value="{{$application_id}}">
                                    <input type="hidden" name="redirect_status" value="{{$status}}">
                                    <input type="hidden" name="status" value="{{$dbStatusArr[$status]}}">
                                    ...
                                    <div class="form-group">
                                        <label style="width: 100%">Mail Subject : @if(isset($data->mail_subject) && $data->mail_body != '')<a href="{{url('/admin/LateSubmission/EditCommunication/preview/email/'.$status.'/'.$application_id)}}" target="_blank" class="btn btn-success" style="float: right !important;">Preview</a>@endif</label>
                                        <div class="">
                                ...
                                @if($status != "ContractLetterText")
                                    <div class="tab-pane fade" id="screen-log">
                                        @include("LateSubmission::letter_log")
                                    </div>
                                    <div class="tab-pane fade" id="email-log">
                                        @include("LateSubmission::email_log")
                                    </div>
                                @endif
                            </div>
            </div>
        ...
        function fetchEmails()
        {
            if($("#mail_body").val() != "" && $("#mail_subjet").val() != "")
            {
                ...
                var status = "{{$dbStatusArr[$status]}}";
                 ...
                 $.ajax({
                    url:'{{url('/admin/LateSubmission/EditCommunication/get/emails/')}}',
                    type:"post",
