...
use App\Modules\Submissions\Models\{Submissions, SubmissionAcademicGradeCalculation, SubmissionCommitteeScore, SubmissionsFinalStatus, SubmissionsStatusLog, SubmissionsStatusUniqueLog, SubmissionTestScore, LateSubmissionFinalStatus};
...
class PreliminaryLateController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($rs as $key => $value) {
            $app_arr[] = $value['id'];
            $tmp = PreliminaryScore::join("submissions", "submissions.id", "submission_preliminary_score.submission_id")->where("application_id", $value['id'])->where("late_submission", "Y")->where("enrollment_id", Session::get("enrollment_id"))->count();
            if ($tmp == 0)
                $prelim_count = 0;
        }
        $exist_count = 0;
        if (count($app_arr) == 0) {
            ...
            $submissions = Submissions::whereIn("application_id", $app_arr)->whereIn("submission_status", array('Active', 'Pending'))->where("late_submission", "Y")->where("enrollment_id", Session::get("enrollment_id"))->count();
            foreach ($app_arr as $k => $v) {
                $exist_count += getPreliminaryScoreLateSubmissionCount($v);
            }
        }
        return view("LateSubmission::Preliminary.index", compact("submissions", "exist_count", "prelim_count"));
    }
    ...
    public function calculate(Request $request)
    {
        ...
        $submissions = Submissions::whereIn("application_id", $app_arr)->whereIn("submission_status", array('Active', 'Pending'))->where("late_submission", "Y")->get();
    ...
    public function commit_score(Request $request)
    {
        ...
        $submissions = Submissions::whereIn("application_id", $app_arr)->where("late_submission", "Y")->whereIn("submission_status", array('Active', 'Pending'))->get();
        ...
        foreach ($submissions as $key => $value) {
            ...
            if ($total_score < $request->thresold_val || $commitee_status == "Fail") {
                ...
                $rs = LateSubmissionFinalStatus::updateOrCreate(["submission_id" => $value->id], $insert);
    ...
    public function roll_back_submissions()
    {
        ...
        foreach ($rs as $key => $value) {
            ...
            $rs1 = LateSubmissionFinalStatus::where("submission_id", $submission_id)->delete();
