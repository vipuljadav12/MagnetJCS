...
<div class="card shadow">
    <div class="card-body d-flex align-items-center justify-content-between flex-wrap">
        <div class="page-title mt-5 mb-5">Edit Prerequisite</div>
...
<form class="" id="preReqForm" action="{{url('admin/ManualSelection/pre_req/update',$data->id)}}" method="post">
    {{csrf_field()}}
    <div class="card shadow">
        <div class="card-body">
            <div class="form-group">
                ...
                <div class="">
                    <span class="form-control custom-select custom-disabled">{{ getProgramName($data->program_id) }}</span>
                    {{-- <select class="form-control custom-select" name="program_id">
                        @forelse($programs as $r=>$program)
                            <option value="{{$program->id}}" @if($program->id == $data->program_id) selected @endif>{{($program->name)}}</option>
                        @empty
                        @endforelse
                    </select> --}}
                </div>
            ...
            <div class="form-group">
                ...
                <div class="">
                    <span class="form-control custom-select custom-disabled">{{ ($data->grade != 0) ? $data->grade : 'All' }}</span>
                    {{-- <select class="form-control custom-select" name="grade">
                        @forelse($grades as $r=>$grade)
                            <option value="{{$grade}}" @if($grade == $data->grade) selected @endif>{{($grade)}}</option>
                        @empty
                        @endforelse
                    </select> --}}
                </div>
            ...
            <div class="form-group">
                ...
                <div class="">
                    @php
                        $course_arr = explode('|', $data->course_name);
                    @endphp
