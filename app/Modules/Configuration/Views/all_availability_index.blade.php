...
@section('title')
    Process Late Submission | {{ config('app.name', env('APP_NAME')) }}
@endsection
@section('content')
    <style type="text/css">
        .buttons-excel {
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item"><a class="nav-link active" id="preview03-tab" data-toggle="tab" href="#preview03" role="tab"
                aria-controls="preview03" aria-selected="true">All Programs</a></li>
        @if ($displayother > 0)
            <li class="nav-item"><a class="nav-link"
                    href="{{ url('/admin/LateSubmission/Population/' . $application_id) }}">Population Changes</a></li>
            <li class="nav-item"><a class="nav-link"
                    href="{{ url('/admin/LateSubmission/Submission/Result/' . $application_id) }}">Submission Results</a></li>
        @endif
    </ul>
...
@section('scripts')
    <div id="wrapperloading" style="display:none;">
        <div id="loading"><i class='fa fa-spinner fa-spin fa-4x'></i> <br> Committing submission status.<br>It will take
            approx 2 minutes to update all records. </div>
    </div>
    ...
    <script type="text/javascript">
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if ($("#process_event").val() != "saveonly")
                        ...
                        "{{ url('/admin/LateSubmission/Population/' . $application_id) }}";
                    $("#process_event").val("");
        ...
        function updateAwardSlot(id) {
            ...
            var withdrawn = parseInt($("#black" + id).val()) + parseInt($("#white" + id).val()) + parseInt($("#other" + id)
                .val());
