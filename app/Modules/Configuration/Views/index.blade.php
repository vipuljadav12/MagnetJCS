...
@section('title')
    Configuration | {{ config('app.name', env('APP_NAME')) }}
@endsection
@section('styles')
    <!-- DataTables -->
    <link
        ...
        <div class="card-body d-flex align-items-center justify-content-between flex-wrap">
            ...
            <div class="">
                @if (checkPermission(Auth::user()->role_id, 'Configuration/create') == 1)
                    <a href="{{ url('admin/Configuration/create') }}" class="btn btn-sm btn-secondary d-none"
                    ...
                    <tbody>
                        @foreach ($configurations as $key => $configuration)
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="{{ url('admin/Configuration/edit', $configuration->id) }}"
                                        class="font-18 ml-5 mr-5" title="Edit"><i class="far fa-edit"></i></a>
                                    <a href="javascript:void(0)" onclick="deletefunction({{ $configuration->id }})"
