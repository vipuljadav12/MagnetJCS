...
@section('title')Process Waitlist | {{config('app.name',env("APP_NAME"))}} @endsection
@section('content')
<style type="text/css">
  ...
  overflow: auto;
    ...
    <div class="card shadow">
        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            @if($displayother > 0)
                <li class="nav-item"><a class="nav-link" href="{{url('/admin/Waitlist/Population/'.$application_id)}}">Population Changes</a></li>
                <li class="nav-item"><a class="nav-link" href="{{url('/admin/Waitlist/Submission/Result/'.$application_id)}}">Submission Results</a></li>
            @endif
        </ul>
        <div class="tab-content bordered" id="myTabContent">
            @include('Waitlist::Template.all_availability')
        </div>
    ...
    <script type="text/javascript">
    	   $('#process_selection').submit(function(event) {
             ...
             var selected = false;
              ...
              $("#wrapperloading").show();
	            $.ajax({
	                url:'{{ url('/admin/Waitlist/Process/Selection/'.$application_id.'/store')}}',
	                type:"POST",
	                data: $('#process_selection').serialize(),
	                success:function(response){
	                   $("#wrapperloading").hide();
                       if($("#process_event").val() != "saveonly")
                            document.location.href = "{{url('/admin/Waitlist/Population/'.$application_id)}}";
                        $("#process_event").val("");
       ...
       /* var dtbl_submission_list = $("#tbl_population_changes").DataTable({"aaSorting": [],
        ...
        function enableDisableWithdrawn(id, val, pos=0)
        {
            if(val == "No")
            {
              ...
              $("#white"+id).val(0);
              $("#other"+id).val(0);
//              $(".hzs_container_"+pos).addClass('hidden');
              $(".hzs_container_"+pos).style.display = 'none';
              updateAwardSlot(id);
            }
            else
            {
              ...
              $(".hzs_container_"+pos).style.display = '';
        ...
        // All Programs TAB - home zone schools
        $(document).ready(function() {

            $('.withdrawn_student').each(function() {
                toggleHomeZoneSchools($(this));
            });
        })
        
        $('.withdrawn_student').change(function() {

            toggleHomeZoneSchools($(this), true);
        });
        function toggleHomeZoneSchools(elm, scroll=false) {
            var application_program_id = elm.data('application_program_id');
            var hzs_container = $('.hzs_container_'+application_program_id);

            if (elm.val() == 'Yes') {
                hzs_container.removeClass('d-none');
                hzs_container.find('input').prop('disabled', false);
            } else {
                
                hzs_container.addClass('d-none');
                hzs_container.find('input').prop('disabled', true);
            }
        }
