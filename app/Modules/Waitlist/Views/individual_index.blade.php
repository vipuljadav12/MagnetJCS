...
@section('title')Individual Program Process Selection | {{config('app.name',env("APP_NAME"))}} @endsection
@section('content')
<style type="text/css">
  ...
  overflow: auto;
    ...
    <div class="card shadow">
        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item"><a class="nav-link " href="{{url('/admin/Waitlist')}}">Form Type</a></li>
            ...
            @if($displayother > 0)
                ...
                <li class="nav-item"><a class="nav-link" href="{{url('/admin/Waitlist/Submission/Result/1')}}">Submission Result</a></li>
            @endif
        </ul>
        <div class="tab-content bordered" id="myTabContent">
            @include('Waitlist::Template.individual_programs')
        </div>
    ...
    <script type="text/javascript">
       ...
       function saveData()
       {
            ...
            $.ajax({
                type: 'get',
                dataType: 'JSON',
                url: "{{url('admin/Waitlist/Individual/Save/')}}/"+program_id+"/"+grade+"/"+seats,
                success: function(response) {
        ...
        function showProgramsWaitlist(val)
        {
            ...
            $.ajax({
                type: 'get',
                dataType: 'JSON',
                url: "{{url('admin/Waitlist/Individual/Show/Response/')}}/"+val,
                success: function(response) {

                        $("#responseprogram").html(response.html);
                        $("#wrapperloading").hide();

                         $("#last_date_waitlist_online_acceptance").datetimepicker({
                            numberOfMonths: 1,
                            autoclose: true,
                             startDate:new Date(),
                        ...
                        $("#last_date_waitlist_offline_acceptance").datetimepicker({
                            numberOfMonths: 1,
                            autoclose: true,
                             startDate:new Date(),
                            dateFormat: 'mm/dd/yy hh:ii',
                         ...
                         $('#process_selection').submit(function(event) {
                            ...
                            if($("#last_date_waitlist_online_acceptance").val() == "")
                            {
                                alert("Please select Last date of online acceptance");
                                return false;
                            }

                            if($("#last_date_waitlist_offline_acceptance").val() == "")
                            {
                            ...
                            $("#wrapperloading").show();
                            $.ajax({
                                url:'{{ url('admin/Waitlist/Individual/store')}}',
                                type:"POST",
                                data: $('#process_selection').serialize(),
                                success:function(response){
                                    $("#wrapperloading").hide();
                                    //alert(response);
                                    document.location.href = "{{url('/admin/Waitlist/Population/Form')}}";
