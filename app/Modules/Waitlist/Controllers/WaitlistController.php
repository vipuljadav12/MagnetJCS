...
class WaitlistController extends Controller
{
    ...
    public function index($application_id = 0)
    {
            ...
            foreach ($program_group as $pk => $pv) {
                if ($pv == $value) {
                    ...
                    if (!empty($exist_process)) {
                        $tmp_data = WaitlistProcessLogs::where("process_log_id", $exist_process->id)->where("program_id", $pk)->first();
                        if (!empty($tmp_data)) {
                            $pdata['visible'] = "Y";
        ...
        return view("Waitlist::all_availability_index", compact("application_id", "disp_arr", "display_outcome", "displayother", "last_date_online_acceptance", "last_date_offline_acceptance", "waitlist_process_logs"));
    }
    ...
    public function population_change_application($application_id = 1, $version = 0)
    {
        ...
        $submissions = Submissions::where("submissions.enrollment_id", Session::get("enrollment_id"))->where('district_id', $district_id)->where(function ($q) {
            ...
            ->where('submissions.enrollment_id', Session::get("enrollment_id"))->where("submissions.form_id", $application_id)->where("submissions_waitlist_final_status.version", $version)->join("submissions_waitlist_final_status", "submissions_waitlist_final_status.submission_id", "submissions.id")
        ...
        foreach ($programs as $program_id => $grades) {
            ...
            if (!empty($availability)) {
                ...
                $rsWait = WaitlistProcessLogs::where("enrollment_id", Session::get("enrollment_id"))->where("program_id", $program_id)->first();
    ...
    public function population_change_version($application_id, $version = 0)
    {
        ...
        foreach ($programs as $program_id => $grades) {
            foreach ($grades as $grade) {
                ...
                if (!empty($availability)) {
                    ...
                    $rsproc = WaitlistProcessLogs::where("enrollment_id", Session::get("enrollment_id"))->where("enrollment_id", Session::get("enrollment_id"))->where("version", $version)->where("application_id", $application_id)->where("program_id", $program_id)->where("grade", $grade)->first();
                    $data = [
                        'program_id' => $program_id,
                        'grade' => $grade,
        ...
        return view("Waitlist::population_change_report", compact('data_ary', 'race_ary', "version", "version_data"));
    }
            ...
            foreach ($gradeArr as $grade) { // -1
                ...
                for ($round = 0; $round <= 2; $round++) { // 0
                    ...
                    foreach ($submissions as $submission) { //1
                        ...
                        if ($submission->{$choice . "_offer_status"} == "Pending" && $submission->{$choice . "_choice_final_status"} == "Waitlisted") // New
                        {
    ...
    public function processWaitlist($req, $application_id, $actual_version, $type = "")
    {
            ...
            if ($req['awardslot' . $value] > 0) {
                ...
                foreach ($submissions as $sk => $sv) {
                    ...
                    if (in_array($sv->third_choice_program_id, $keys) && !in_array($sv->first_choice_program_id, $keys) && !in_array($sv->second_choice_program_id, $keys)) {
                        ...
                        if ($sv->third_choice_final_status != "Waitlisted") {
        ...
        $displayRecords = [];
        if ($type == "update") {
            $rs = SubmissionsWaitlistFinalStatus::where("enrollment_id", Session::get("enrollment_id"))->where("version", $version)->where("application_id", $application_id)->delete();
        ...
        foreach ($submissions as $submission) {
            ...
            if ($type == "update") {
                ...
                $rs = SubmissionsWaitlistFinalStatus::updateOrCreate(["submission_id" => $submission->id], $data);
    ...
    public function storeAllAvailability(Request $request, $application_id)
    {
        ...
        if ($process && $req['process_event'] != "saveonly") {
            ...
            $test = $this->processWaitlist($req, $application_id, $version, $type);
        }
    ...
    public function submissions_results_application($application_id = 59, $version = 0)
    {
        ...
        foreach ($submissions as $key => $value) {
            ...
            if ($value->thirf_choice_program_id != 0) {
                $tmp = array();
                $tmp['id'] = $value->id;
                $tmp['name'] = $value->first_name . " " . $value->last_name;
                $tmp['grade'] = $value->next_grade;
                ...
                $tmp['race'] = $value->calculated_race;
                $tmp['choice'] = 3;
                $tmp['program'] = getProgramName($value->thirf_choice_program_id) . " - Grade " . $value->next_grade;
