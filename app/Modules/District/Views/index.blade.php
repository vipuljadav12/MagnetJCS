...
@section('content')
    <div class="card shadow">
        <div class="card-body d-flex align-items-center justify-content-between flex-wrap">
            ...
            <div class="">
                <a href="{{ url('admin/District/create') }}" class="btn btn-sm btn-secondary" title="">Add
                    District</a>
                        ...
                        @foreach ($districts as $key => $district)
                            <tr>
                                <td class="align-middle text-center">
                                    <img src="{{ url('/resources/filebrowser/') . '/' . $district->district_slug . '/logo/' . $district->district_logo }}"
                                        alt="img" title="" width="70" id="img"
                                ...
                                <td>
                                    <a href="{{ 'http://' . '/' . $district->district_slug . '.' . Request::getHost() }}"
                                        target="_blank">{{ $district->district_slug }}.{{ Request::getHost() }}</a>
                                </td>
                                <td class="text-center">
                                    <input id="{{ $district->id }}" type="checkbox"
                                        class="js-switch js-switch-1 js-switch-xs status" data-size="Small"
                                        {{ $district->status == 'Y' ? 'checked' : '' }} />
                                </td>
                                <td class="text-center">
                                    <a href="{{ url('admin/District/edit', $district->id) }}" class="font-18 ml-5 mr-5"
                                        title=""><i class="far fa-edit"></i></a>
                                    <a href="javascript:void(0)" onclick="deletefunction({{ $district->id }})"
